...
prs ∷ ∀ a r. RouteDuplex' r a → RouteParser r a
prs (RouteDuplex _ p) = p

prt ∷ ∀ a r. RouteDuplex' r a → (a → RoutePrinter r)
prt (RouteDuplex p _) = p

class VariantParser (rl ∷ RowList) (routes ∷ #Type) (variantRoute ∷ #Type) ctx | routes → variantRoute where
  variantParser ∷ RLProxy rl → Record routes → RouteParser ctx (Variant variantRoute)

instance variantParserNil ::
  (IsSymbol sym, Row.Cons sym (RouteDuplex' ctx a) r' r, Row.Cons sym a v' v) =>
  VariantParser (RowList.Cons sym (RouteDuplex ctx a a) RowList.Nil) r v ctx where
  variantParser _ r = inj prop <$> prs (Record.get prop r)
    where
    prop = SProxy ∷ SProxy sym
else instance variantParserCons ∷
  (IsSymbol sym, VariantParser tail r v ctx, Row.Cons sym (RouteDuplex' ctx a) r' r, Row.Cons sym a v' v) =>
  VariantParser (RowList.Cons sym (RouteDuplex ctx a a) tail) r v ctx where
  variantParser _ r = inj prop <$> prs (Record.get prop r) <|> variantParser (RLProxy ∷ RLProxy tail) r
    where
    prop = SProxy ∷ SProxy sym

class VariantPrinter (rl ∷ RowList) (routes ∷ #Type) (variantRoute ∷ #Type) ctx | rl → routes, rl → variantRoute where
  variantPrinter ∷ RLProxy rl → Record routes → Variant variantRoute → RoutePrinter ctx

instance variantPrinterNil ::
  VariantPrinter RowList.Nil r () ctx where
  variantPrinter _ _ = case_
else instance variantPrinterCons ∷
  (IsSymbol sym, VariantPrinter tail r v' ctx, Row.Cons sym (RouteDuplex' ctx a) r' r, Row.Cons sym a v' v) =>
  VariantPrinter (RowList.Cons sym (RouteDuplex ctx a a) tail) r v ctx where
...
variant ∷
  ∀ ctx r rl v.
  RowToList r rl ⇒
  VariantParser rl r v ctx ⇒
  VariantPrinter rl r v ctx ⇒
  Record r →
  RouteDuplex' ctx (Variant v)
...
newtype Updater r
  = Updater (Record.Builder.Builder r r)
...
instance prefixRoutesEmptyCons ::
  (PrefixRoutes tail routes, Row.Cons "" (RouteDuplex ctx a a) r' routes) =>
  PrefixRoutes (RowList.Cons "" (RouteDuplex ctx a a) tail) routes where
  prefixRoutes _ = prefixRoutes (RLProxy ∷ RLProxy tail)
else instance prefixRoutesCons ::
  (IsSymbol sym, PrefixRoutes tail routes, Row.Cons sym (RouteDuplex ctx a a) r' routes) =>
  PrefixRoutes (RowList.Cons sym (RouteDuplex ctx a a) tail) routes where
  prefixRoutes _ = modify prop (prefix (reflectSymbol prop)) <> prefixRoutes (RLProxy ∷ RLProxy tail)
    where
    prop = SProxy ∷ SProxy sym

class (VariantParser rl r v ctx, VariantPrinter rl r v ctx, PrefixRoutes rl r) ⇐ Variant' (rl ∷ RowList) (r ∷ #Type) (v ∷ #Type) ctx

instance variantParser' ∷ (VariantParser rl r v ctx, VariantPrinter rl r v ctx, PrefixRoutes rl r) ⇒ Variant' rl r v ctx
